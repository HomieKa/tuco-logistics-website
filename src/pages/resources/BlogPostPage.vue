<template>
  <article class="bg-white py-16">
    <div class="container mx-auto max-w-4xl px-4">
      <RouterLink to="/resources/insights" class="mb-6 inline-flex items-center text-sm font-semibold text-sky-500 hover:text-sky-600">
        ← Back to insights
      </RouterLink>
      <h1 class="text-4xl font-bold text-navy-500">{{ post.title }}</h1>
      <p class="mt-4 text-sm font-semibold uppercase tracking-wide text-teal-500">{{ post.category }}</p>
...
const articles = {
  ...
  "invoice-automation": {
    ...
    category: "Finance",
    excerpt: "Turn error-prone manual work into a streamlined finance process with Freightmate and TUCO oversight.",
    body: [
